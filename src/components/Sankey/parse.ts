...
export const colorRectFunc = (dataPoint: RectNode) => d3Color(dataPoint.category || dataPoint.name);

export const colorLinkFunc = (dataPoint: any) => {
  const name = typeof dataPoint.target === 'object' ? dataPoint.target?.name : dataPoint.target;
...
export const formatLinkTitleFunc = ({ source, target, value }: any): string => {
...
export const makeSankeyFunc = ({
  width,
  height,
}: MakeSankeyInput): SankeyLayout<SankeyData, SankeyDataNode, SankeyDataLink> => {
