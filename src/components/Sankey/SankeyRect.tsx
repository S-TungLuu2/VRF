...
export const SankeyRect = ({ color, title, ...rectProps }: SankeyRectProps): JSX.Element => {
  return (
...
export const SankeyRects = ({ nodes, titleFunc, colorFunc }: SankeyRectsProps): JSX.Element => {
  return (
    <g stroke="#000">
      {nodes.map((node) => {
        const coords = getCoordinates(node);
        return <SankeyRect {...coords} key={node.name} color={colorFunc?.(node)} title={titleFunc?.(node)} />;
      })}
    </g>
  );
};
